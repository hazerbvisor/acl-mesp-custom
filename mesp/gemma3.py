...
class LazyLoadingAttention(Attention, LazyLoadingMixin):
    def __init__(self, args: QLoRAModelArgs, layer_idx: int):
        ...
        self.q_proj = QuantizedLinear(
            input_dims=dim, output_dims=n_heads * head_dim, **common_kwargs
        )
        self.k_proj = QuantizedLinear(
            input_dims=dim, output_dims=n_kv_heads * head_dim, **common_kwargs
        )
        self.v_proj = QuantizedLinear(
            input_dims=dim, output_dims=n_kv_heads * head_dim, **common_kwargs
        )
        self.o_proj = QuantizedLinear(
            input_dims=n_heads * head_dim, output_dims=dim, **common_kwargs
        )
...
class LazyLoadingMLP(MLP, LazyLoadingMixin):
    def __init__(self, args: QLoRAModelArgs):
        ...
        self.gate_proj = QuantizedLinear(
            input_dims=dim, output_dims=hidden_dim, **common_kwargs
        )
        self.down_proj = QuantizedLinear(
            input_dims=hidden_dim, output_dims=dim, **common_kwargs
        )
...
class LazyLoadingTransformerBlock(TransformerBlock, LazyLoadingMixin):
    def __init__(self, args: QLoRAModelArgs, layer_idx: int):
        ...
        self.self_attn = LazyLoadingAttention(args, layer_idx)
...
class LazyLoadingLMLoss(nn.Module, LazyLoadingMixin):
    ...
    def __call__(
        self, x: mx.array, targets: mx.array
    ) -> mx.array | tuple[mx.array, ...]:
        logits = self.linear(self.norm(x))
        mask = mx.logical_and(targets != NULL_LABEL_VALUE, targets != PAD_VALUE).astype(
            logits.dtype
        )
...
def export_embedding(
    ...
    assert (
        "x" in variable_names
        and "y" in variable_names
        and isinstance(module, nn.QuantizedEmbedding)
    )
    ...
    # Gemma3 scaling factor with correct dtype
    scaling_factor = mx.array(args.hidden_size**0.5, dtype=dtype)
    ...
    def fn(*inputs):
        ...
        return output.astype(intermediate_dtype).astype(target_dtype) * scaling_factor

    example_x = mx.random.randint(
        low=0, high=args.vocab_size, shape=(batch_size, context_length)
    )
    ...
    return RunFunctionConfig(
        ...
        input_names=[variable_names["x"]]
        + [param_name_formatter(n) for n in flatten_param_names],
...
def export_transformer_block(
    ...
    assert layer_idx >= 0
    trainable_param_names, trainable_params = zip(
        *tree_flatten(module.trainable_parameters())
    )
    ...
    lazy_module = LazyLoadingTransformerBlock(args, layer_idx)
...
def export_loss(
    ...
    assert (
        "x" in variable_names
        and "targets" in variable_names
        and isinstance(module, Gemma3Model)
    )
    linear_param_names, linear_params = zip(
        *tree_flatten(module.embed_tokens.parameters())
    )
    ...
    example_x = mx.random.normal(
        shape=(batch_size, context_length, args.hidden_size), dtype=dtype
    )
    example_targets = mx.random.randint(
        low=0, high=args.vocab_size, shape=(batch_size, context_length)
    )
    ...
    flatten_param_names = list(linear_param_names) + [
        f"norm.{n}" for n in norm_param_names
    ]
    return RunFunctionConfig(
        function_name=module_name,
        input_names=[variable_names["x"], variable_names["targets"]]
        + flatten_param_names,
